...
static void video_channel_message_broadcast(video_channel_publisher_t *publisher, video_message_t *message) {
    ...
    for (size_t i = 0; i < publisher->channel->subscribers.size; i++) {
        ...
        video_queue_enqueue(&subscriber->queue, message, video_slab_unref);
...
void video_channel_publish(video_channel_publisher_t *publisher, video_channel_t *channel, video_thread_t *thread) {
    ...
    message->len = 0;
    message->time = 0;
    message->type = VIDEO_MESSAGE_TYPE_BEGIN;
    video_channel_message_broadcast(publisher, message);
}

void video_channel_unpublish(video_channel_publisher_t *publisher) {
    uv_mutex_lock(&publisher->channel->mutex);
    publisher->state = VIDEO_CHANNEL_PUBLISHER_STATE_IDLE;
    publisher->since = time(NULL);
    publisher->channel->publisher = 0;
    uv_mutex_unlock(&publisher->channel->mutex);
    ...
    message->len = 0;
    message->time = 0;
    message->type = VIDEO_MESSAGE_TYPE_DONE;
    video_channel_message_broadcast(publisher, message);
}
